<script setup>
import { IconAlertTriangle, IconX } from '@tabler/icons-vue';

defineProps({
    show: { type: Boolean, default: false },
    message: { type: String, default: '' },
    strike: { type: Number, default: 0 },
});

defineEmits(['close', 'back']);
</script>

<template>
    <Teleport to="body">
        <div v-if="show" class="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-6 backdrop-blur-sm">
            <div class="bg-surface-white rounded-3xl p-8 shadow-app-frame max-w-md w-full">
                <div class="flex items-start gap-4">
                    <div class="w-12 h-12 bg-amber-100 rounded-full flex items-center justify-center shrink-0">
                        <IconAlertTriangle class="text-amber-600 dark:text-amber-400" :size="24" />
                    </div>
                    <div class="flex-1">
                        <h3 class="text-title-lg font-semibold text-text-heading mb-1">Peringatan Ke-{{ strike }}</h3>
                        <p class="text-text-body text-body-md">{{ message || 'Aktivitas mencurigakan terdeteksi.' }}</p>
                        <p class="text-error-red text-label-md mt-2 font-medium">3 pelanggaran = sesi dihentikan.</p>
                        <button
                            @click="$emit('back')"
                            class="mt-4 px-5 py-2.5 rounded-full text-label-md font-semibold bg-primary-container text-white hover:bg-primary transition-all shadow-md active:scale-95 duration-150"
                        >
                            Kembali ke Ujian
                        </button>
                    </div>
                    <button @click="$emit('close')" class="text-text-muted hover:text-text-heading transition-colors shrink-0">
                        <IconX :size="20" />
                    </button>
                </div>
            </div>
        </div>
    </Teleport>
</template>
