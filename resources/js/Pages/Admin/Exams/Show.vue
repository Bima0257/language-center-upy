...
async function deleteGroup(groupId) {
    if (await confirm.confirm('Hapus grup soal ini? Semua soal di dalamnya akan ikut terhapus.')) {
        const form = useForm({});
...
async function deleteSection(sectionId) {
    if (await confirm.confirm('Hapus section ini? Semua grup dan soal di dalamnya akan terhapus.')) {
        const form = useForm({});
                    ...
                    <div v-for="section in exam.sections" :key="section.id"
                        ...
                        <div v-if="expandedSections[section.id]" class="p-4 space-y-4 border-t border-outline-variant/30">
                                    ...
                                    <div v-if="group.questions?.length" class="space-y-2 mb-3">
                                        <div v-for="q in group.questions" :key="q.id"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-text-body text-body-md text-primary truncate">{{ q.question_text }}</p>
                            ...
                            <div v-else class="text-center py-6">
                                <p class="text-text-muted text-text-body text-body-md">Belum ada grup soal. Buat grup untuk mulai menambahkan soal.</p>
                            </div>
                            ...
                            <form @submit.prevent="addGroup(section.id)" class="border-t border-outline-variant/30 pt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div class="flex gap-2">
                                        <input type="number" v-model="groupForm.order" placeholder="Urutan" min="1"
                                               class="w-24 px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded-2xl text-text-body text-body-md focus:outline-none focus:border-secondary" />
                                ...
                                <textarea v-model="groupForm.passage_text" rows="3" placeholder="Passage text (untuk Reading section)"
                                          class="mt-3 w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded-2xl text-text-body text-body-md focus:outline-none focus:border-secondary"></textarea>
                            </form>
                        </div>
                    </div>
                </div>
